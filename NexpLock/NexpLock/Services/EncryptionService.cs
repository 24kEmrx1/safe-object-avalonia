...
public sealed class EncryptionService(IStorageService storageService) : IEncryptionService
{
        ...
        IProgress<(double percentage, string message)> progress, CancellationToken token)
    {
        ...
        await storageService.EncryptFileAsync(
            new FileProcessingRequest(fileId, sourcePath, encryptedFilePath), key, token, progress);
        ...
        IProgress<(double percentage, string message)> progress, CancellationToken token)
    {
        ...
        await storageService.DecryptFileAsync(
            new FileProcessingRequest(fileId, sourcePath, decryptedFilePath), key, token, progress);
    ...
    {
        return new DirectStream(path, mode, access, FileShare.None, Constants.Encryption.BufferSize,
