...
using static NexpSafe.Kernel.LinuxKernel;
using static NexpSafe.Kernel.WindowsKernel;
...
public sealed class DirectStream : FileStream
{
    ...
    {
        return _isWindows
            ? new NativeHandles(0, IntPtr.Zero)
    ...
    {
        if (_isWindows is not true)
            ConfigureUnixStreamProperties();
    }
